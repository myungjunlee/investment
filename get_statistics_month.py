...
column_list = ['년도', '월', '유형', '승','패','승률','평균이익', '평균손실', '손익비', '수익률','최대손실', '최대낙폭']
...
for i in range(2017,2025):
    ...
    if (i != 2024):
        ...
        filter_df = filter[filter['날짜'].between(str(i)+'-01-01', str(i)+'-12-31')]

    else:
        filter_df = filter

    for k in range(1,13):
        filter2_df = filter_df[filter_df['날짜'].dt.month == k]
        long_df = filter2_df[filter2_df['롱/숏'].isin(['long'])]
        short_df = filter2_df[filter2_df['롱/숏'].isin(['short'])]

        for j in range(0,3):
            if j == 0:
                season_df = filter2_df
                option = "롱/숏"
            elif j == 1:
                season_df = long_df
                option = '롱'
            ...
            if i == 2024:
                i = '모든 년도'
            stat_list = [i, k, option, win, lost, win_lost,average_profit, average_loss, profit_loss, hpr, mdd, max_low]
