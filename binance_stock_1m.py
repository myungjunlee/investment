...
con = sqlite3.connect("./minute1.db")
...
def ohlcv_data():
    ...
    while True:
        if input_date == 0:
            ...
            start_date = to_mstimestamp(str(input_date + datetime.timedelta(minutes=1)))
        # end_date = to_mstimestamp('2017-12-31 23:59:59')
        ohlcv = binance.fetch_ohlcv('BTC/USDT', timeframe='1m', since=start_date, limit=1000)
...
print(trading_df)
trading_df.to_sql("bitcoin1m", con, index=False, if_exists='replace')
